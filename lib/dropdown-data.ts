...
export const subjectData: Record<string, Record<string, string[]>> = {
    'CSE': {
...
// Helper functions
export const getColleges = (): string[] => Object.keys(collegeData).sort();

export const getBranchesForCollege = (college: string): string[] => {
    return collegeData[college] || [];
};

export const getSubjectsForBranchAndSemester = (branch: string, semester: string): string[] => {
    return subjectData[branch]?.[semester] || [];
};

export const getAllBranches = (): string[] => {
    const branches = new Set<string>();
    Object.values(collegeData).forEach(branchList => {
        branchList.forEach(branch => branches.add(branch));
    });
...
export const getAllSubjects = (): string[] => {
    ...
    Object.values(subjectData).forEach(semesterData => {
        Object.values(semesterData).forEach(subjectList => {
